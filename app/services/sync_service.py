...
def _newer_than(a: str, b: str, seconds: int) -> bool:
    """True when ISO timestamp `a` is at least `seconds` newer than `b`."""
    if not a or not b:
        return False
    try:
        a_dt = datetime.strptime(a, "%Y-%m-%dT%H:%M:%S")
        b_dt = datetime.strptime(b, "%Y-%m-%dT%H:%M:%S")
        return (a_dt - b_dt).total_seconds() >= seconds
    except ValueError:
        return False
        ...
        try:
            ...
            for row in remote_conn.execute("SELECT * FROM words"):
                ...
                if local_row is None:
                    ...
                    local_deleted = bool(local_word.get("deleted_at"))
                    remote_deleted = bool(remote_word.get("deleted_at"))
                    if local_deleted != remote_deleted:
                        deleted_ua = local_updated if local_deleted else remote_updated
                        active_ua = remote_updated if local_deleted else local_updated
                        remote_wins = _newer_than(active_ua, deleted_ua, 5)
                    else:
                        remote_wins = remote_updated > local_updated
