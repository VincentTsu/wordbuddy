...
final class WordDbHelper extends SQLiteOpenHelper {
        ...
        try (Cursor c = remote.rawQuery("SELECT * FROM words", null)) {
            for (Word remoteWord : readWords(c)) {
                ...
                if (localWord == null) {
                    ...
                    // Both have it – later updated_at wins, but deletion has priority:
                    // if exactly one side is soft-deleted, the deletion wins unless
                    // the non-deleted side is meaningfully newer (user re-added on another device).
                    boolean localDeleted = !localWord.deletedAt.isEmpty();
                    boolean remoteDeleted = remoteWord.deletedAt != null && !remoteWord.deletedAt.isEmpty();

                    boolean remoteWins;
                    if (localDeleted != remoteDeleted) {
                        String deletedUA = localDeleted ? localWord.updatedAt : remoteWord.updatedAt;
                        String activeUA = localDeleted ? remoteWord.updatedAt : localWord.updatedAt;
                        remoteWins = newerBy(activeUA, deletedUA, 5);
                    } else {
                        remoteWins = remoteWord.updatedAt.compareTo(localWord.updatedAt) > 0;
                    }
    ...
    /** True when ISO timestamp {@code a} is at least {@code seconds} newer than {@code b}. */
    private static boolean newerBy(String a, String b, int seconds) {
        if (a == null || a.isEmpty() || b == null || b.isEmpty()) return false;
        try {
            SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", java.util.Locale.US);
            long epochA = fmt.parse(a).getTime() / 1000;
            long epochB = fmt.parse(b).getTime() / 1000;
            return (epochA - epochB) >= seconds;
        } catch (Exception e) {
            return false;
        }
    }
