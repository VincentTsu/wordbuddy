...
public class MainActivity extends Activity {
    ...
    private void showHome() {
        ...
        statsCard.addView(statItem(String.valueOf(mastered), "已掌握", GREEN));
        statsCard.addView(statDivider());
    ...
    private void showWordDetail(int id) {
        ...
        root.addView(ghostButton("返回词库", v -> showLibrary(searchInput == null ? "" : searchInput.getText().toString())));
    }
    ...
    private void showNextReview() {
        ...
        // Progress
        int total = reviewQueue.size();
        TextView progress = new TextView(this);
        progress.setText("复习中 · 剩余 " + total + " 个");
        ...
        // Definition card - hidden initially, tap to reveal
        ...
        defCard.setOnClickListener(v -> {
            if (!revealed[0]) {
                ...
                // Show examples too
                if (!w.englishDefinition.isEmpty()) {
                    TextView engDef = new TextView(this);
        ...
        spacer5.setLayoutParams(new LinearLayout.LayoutParams(1, dp(32)));
        box.addView(spacer5);
        ...
        box.addView(reviewButton("记住了", GREEN, v -> review(w.id, "remembered")));
        box.addView(reviewButton("有点模糊", AMBER, v -> review(w.id, "fuzzy")));
        box.addView(reviewButton("没记住", RED, v -> review(w.id, "forgotten")));
    ...
    private Button reviewButton(String text, String color, View.OnClickListener listener) {
        ...
        bg.setStroke(dp(1), Color.parseColor(color + "40"));
        b.setBackground(bg);
    ...
    private Button dangerButton(String text, View.OnClickListener listener) {
        ...
        GradientDrawable bg = new GradientDrawable();
        bg.setCornerRadius(dp(12));
        bg.setColor(Color.parseColor(RED + "12"));
        bg.setStroke(dp(1), Color.parseColor(RED + "30"));
        b.setBackground(bg);
        b.setOnClickListener(listener);
        return b;
    }
